...
def save_model_and_tokenizer(model, base_model_path, output_path, remote_path):
    ...
    for fp in os.listdir(remote_path):
        # 拷贝remote_scripts中官方脚本到最终输出的文件夹中，供load模型时使用
        if fp.split('.')[-1] == 'py':
...
def main(lora_path, output_path, remote_path, qbits=4, device_map=None):
    ...
    if qbits in [4,8]:
        quantize_model = quantize(merged_model, qbits)
        save_model_and_tokenizer(quantize_model, lora_config.base_model_name_or_path,output_path,remote_path)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Llama-7B merge lora and quantize.')
    ...
    parser.add_argument('--remote_scripts_dir', type=str, default='', help='官方脚本目录')
